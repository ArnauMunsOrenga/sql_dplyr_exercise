...
```{r}
DATA_DIR <- file.path("..", "football_data") #los dos puntos nos llevan una carpeta encima
library(RSQLite)
sql_con <- dbConnect(
  SQLite(), 
...
#el show_query lo que nos permite es traducir a sql una consulta que hagamos en formato dplyr
```
...
LEFT JOIN -> NOS MANTIENE TODO LO QUE HAYA EN LA TABLA DE LA IZQUIERDA
...
#aqui nos esta haciendo una lazy query que significa que la query no se ejecuta hasta que no hagamos un head o algo a la tabla que nos guardamos de la consulta

#la consulta no se ejecuta hasta que no le decimos que nos muestre el resultado
#esto hace que no se tenga que ejecutar cada consulta hasta el final y nos otpimiza el proceso

player_db <- tbl(sql_con, "Player")
player_attributes_db <- tbl(sql_con, "Player_Attributes")

players_rank<-player_db %>%
  left_join(
...
simplemente lo que nos hace es juntar dos tablas a pelo, aunque haya cosas repetidas, despues podemos filtrar
por ejemplo si tenemos en una tabla los goles como visitante y en otra los goles como local
...
tbl(sql_con, "Match") %>% select(country_id) %>% group_by(country_id) %>% count %>% arrange(desc(n))
